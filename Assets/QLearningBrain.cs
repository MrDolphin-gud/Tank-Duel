...
public class QLearningBrain : MonoBehaviour
{
    ...
    void Awake()
    {
        // Öğrenilen modeli persistentDataPath'e kaydet (Assets'teki hazır modeli değiştirmemek için)
        // Hazır model: Assets/RLBrain.json (sadece okuma için)
        // Öğrenilen model: persistentDataPath/RLBrain.json (yazma için)
        savePath = Path.Combine(Application.persistentDataPath, saveFileName);
        
        Debug.Log($"[QLearningBrain] Save path (ogrenilen model): {savePath}");
    }
    ...
    void Start()
    {
        ...
        // Model'i yükle (WebGL için coroutine)
        StartCoroutine(LoadOrCreateModelCoroutine());
    }
    
    private IEnumerator LoadOrCreateModelCoroutine()
    {
        yield return StartCoroutine(LoadOrCreateModelAsync());
    ...
    private IEnumerator LoadOrCreateModelAsync()
    {
        ...
        // AI yükleme aktif mi kontrol et (varsayılan: açık)
        bool shouldLoadAI = PlayerPrefs.GetInt("LoadAI", 1) == 1;
        
        if (!shouldLoadAI)
        {
            // AI yükleme kapalı, yeni model oluştur (önce mevcut öğrenilmiş modeli yedekle)
        ...
        // WebGL için StreamingAssets'ten UnityWebRequest ile yükle
        #if UNITY_WEBGL && !UNITY_EDITOR
        string streamingAssetsUrl = Path.Combine(Application.streamingAssetsPath, saveFileName);
        Debug.Log($"[QLearningBrain] WebGL: StreamingAssets'ten yukleniyor: {streamingAssetsUrl}");
        
        using (UnityWebRequest www = UnityWebRequest.Get(streamingAssetsUrl))
        {
            ...
            if (www.result == UnityWebRequest.Result.Success)
            {
                ...
                loadPath = streamingAssetsUrl;
                Debug.Log($"[QLearningBrain] Hazir model yuklendi (WebGL StreamingAssets): {loadPath}");
            }
            else
            {
                Debug.Log($"[QLearningBrain] StreamingAssets'ten yukleme basarisiz: {www.error}");
            }
        }
        #else
        // Editor ve diğer platformlar için normal dosya sistemi
        string streamingAssetsPath = Path.Combine(Application.streamingAssetsPath, saveFileName);
        
        // Editor'da Assets klasörünü de kontrol et (StreamingAssets yoksa)
        #if UNITY_EDITOR
        string assetsModelPath = Path.Combine(Application.dataPath, saveFileName);
        if (File.Exists(assetsModelPath))
        {
            // Editor'da Assets klasöründeki hazır modeli yükle
            loadPath = assetsModelPath;
            Debug.Log($"[QLearningBrain] Hazir model yukleniyor (Assets): {loadPath}");
        ...
        {
            // StreamingAssets'teki hazır modeli yükle (Build'de çalışır)
            loadPath = streamingAssetsPath;
            Debug.Log($"[QLearningBrain] Hazir model yukleniyor (StreamingAssets): {loadPath}");
            json = File.ReadAllText(loadPath);
        }
        #endif
        
        // Hazır model yüklenemediyse öğrenilmiş modeli kontrol et
        if (string.IsNullOrEmpty(json) && File.Exists(savePath))
        {
            // Hazır model yok ama öğrenilmiş model var, onu yükle
            loadPath = savePath;
            Debug.Log($"[QLearningBrain] Ogrenilmis model yukleniyor: {loadPath}");
            json = File.ReadAllText(loadPath);
        }
        ...
        {
            // Hiç model yok, yeni model oluştur
            Debug.Log("[QLearningBrain] Model bulunamadi. Yeni model olusturuluyor.");
            CreateNewModel();
        }
    }

    private void BackupLearnedModel()
    {
        // Öğrenilmiş model varsa yedekle 
        if (File.Exists(savePath))
        {
            try
            {
                string timestamp = System.DateTime.Now.ToString("yyyyMMdd_HHmmss");
                string backupFileName = $"RLBrain_Backup_{timestamp}.json";
                string backupPath = Path.Combine(Application.persistentDataPath, backupFileName);
                
                File.Copy(savePath, backupPath, true);
                Debug.Log($"[QLearningBrain] Ogrenilmis model yedeklendi: {backupPath}");
            }
            catch (System.Exception e)
            {
                Debug.LogWarning($"[QLearningBrain] Yedekleme hatasi: {e.Message}");
            }
        }
    }
    ...
    {
        // Yeni model oluşturulmadan önce mevcut öğrenilmiş modeli yedekle
        BackupLearnedModel();
